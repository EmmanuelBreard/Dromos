...
### Fixed
- **Plan generation 401 on stale session** — `PlanService.generatePlan()` now refreshes the session token before invoking the edge function. If the refresh token is expired, the user gets a clear "Your session has expired. Please sign in again." message instead of a generic HTTP 401 error. Also fixed a stale code comment in `generate-plan` that incorrectly claimed `--no-verify-jwt` was set (DRO-195)
