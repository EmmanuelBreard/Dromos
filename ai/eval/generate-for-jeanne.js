...
// ── User profile (jeannedelaharpe@gmail.com) ───────────────────────────────────
const USER_ID = "8c60a722-7155-4f45-add6-ff3913ff6be8";
const userProfile = {
  id: USER_ID,
  race_objective: "Olympic",
  race_date: "2026-09-20T19:32:00+00:00",
  vma: 17.0,
  ftp: 125,
  experience_years: 1,
  current_weekly_hours: 3.0,
  css_seconds_per100m: 140,
  // Monday forced to REST (cap 0, no eligible sports) so every week keeps a rest day.
  swim_days: ["Thursday","Saturday","Sunday"],
  bike_days: ["Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],
  run_days:  ["Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],
  mon_duration: 0,
  tue_duration: 60,
  wed_duration: 60,
  thu_duration: 60,
...
async function main() {
  ...
  // DRO-285 ramp: anchor the planning ceiling to current load, NOT raw availability (12h).
  // A ~10%/week ramp from 3h with recovery weeks peaks around 6.5h — use that as the ceiling.
  const availabilityHours = calculateWeeklyHours(userProfile);
  const weeklyHours = Math.min(availabilityHours, 6.5);
  ...
  console.log(`\n=== Generating plan for jeannedelaharpe@gmail.com ===`);
...
## ADDITIONAL HARD CONSTRAINTS (override any conflicting rule above)
- This athlete currently trains only ${userProfile.current_weekly_hours}h/week. Week 1 TOTAL volume MUST be ~${userProfile.current_weekly_hours}h (do NOT exceed 3.5h).
- Increase total weekly volume by NO MORE than ~10% versus the previous loading week.
- Recovery weeks reduce total volume 30-50% from the prior loading week.
- Peak loading weeks MUST NOT exceed ${vars.weeklyHours}h total. Do NOT plan toward raw availability — this athlete cannot absorb it yet.
- Monday is a REST day: schedule NOTHING on Monday. Every week MUST keep at least one full rest day.`;
  const step1Output = await callOpenAI(MODEL_STEP1, buildStep1Prompt(userProfile, vars) + RAMP_OVERRIDE, TEMPERATURE, MAX_TOKENS_STEP1);
