...
| `ai/eval/run-generation-eval.js` | DRO-315 (Phase 3): generation eval runner. For each scenario in `vars/availability-scenarios.yaml`, runs N iterations (default 3) through the real deployed `generate-plan`: `createTestUser` → `invokeGeneratePlan` → `pollStatus` → `readPlan` → `dbPlanToEvalShape` → `scorePlan` → classify HARD/SOFT. A scenario FAILS on any HARD violation in any run, OR if all runs are generation-reliability failures (poll `failed`/`timeout`, tracked separately from quality failures). Per-metric stability labeled via `aggregate-violations.js`'s shared `labelStability`. Guaranteed `finally` cleanup of every test user (+ a crash-safety sweep). Emits a structured results object + writes `results/generation-eval-<label>.json`. **DRO-317 (Phase 5):** each scored run also gets an inline advisory Yupa audit (`reviewPlan`) stored as `run.coaching` — strictly non-gating and fault-tolerant (an OpenAI failure only downgrades that run's `coaching.verdict` to `UNKNOWN`); and `main()` renders the markdown report via `lib/report.js` at the end. The Supabase client is lazy-loaded so the offline `--report` re-render needs no credentials. CLI flags: `--scenarios <N>` (first N), `--runs <N>` (override N=3), `--label <tag>` (output filename), `--report <results.json>` (re-render the markdown report from an existing results JSON — pure/offline, no DB or OpenAI). |
| `ai/eval/lib/report.js` | DRO-317 (Phase 5): report renderer for the generation eval. `buildReport(results)` → markdown string (pure: no network/DB/filesystem); `writeReport(results, outDir)` → writes `results/eval-report-<runLabel>.md` and returns the path. Renders the header/config, overall PASS/FAIL summary (+ any un-cleaned-up test users ⚠️), and per scenario: verdict, HARD/SOFT-per-run breakdown, per-metric stability labels, generation-reliability failures with `error_message`, and the advisory Yupa coaching verdict/issues. |
| `ai/eval/run-eval.sh` | DRO-317 (Phase 5): one-command entry point for the real-edge-function generation eval. Runs `run-generation-eval.js` with all flags passed through (`--scenarios`/`--runs`/`--label`), which scores, runs the advisory Yupa audit inline, writes the results JSON, and renders the markdown report; the script then prints the report path. ⚠️ Real prod cost per run (a `generate-plan` call + one gpt-4.1 audit per scored plan) — use `--scenarios`/`--runs` to keep it cheap. |

> **Two eval tracks, do not conflate:** `run-generation-eval.js` + `run-eval.sh` are the **real-edge-function generation eval** — they exercise the actual deployed `generate-plan` against production (with strict test-user cleanup) and score the materialized DB plan. This is distinct from the **shadow promptfoo pipeline** (`promptfooconfig*.yaml`, `run-step3-blocks.js`, `athletes.yaml`), which evaluates the prompt/step logic in isolation without hitting the deployed function.
