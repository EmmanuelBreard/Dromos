...
### Database Migrations
- **017_plan_snapshots.sql** — Adds `plan_snapshots` table (snapshot JSONB, reason, RLS SELECT-own) for safe pre-replace plan archival. Adds `import_plan_atomic` SECURITY DEFINER function with `search_path = public` lockdown and execution restricted to `service_role` (no anon / authenticated). Function handles the atomic snapshot → CASCADE delete → insert flow for plan imports (DRO-299).
- **018_revoke_import_plan_atomic_from_public.sql** — Defense-in-depth: explicit `REVOKE EXECUTE` from PUBLIC/anon/authenticated on `import_plan_atomic`. Also pins `race_date::date::timestamptz` cast (UTC midnight, timezone-stable) (DRO-299).

### Added
- **Unscheduled sessions now show up on Home and Calendar** — Activities you do that aren't in your plan (an extra ride, a recovery run on a rest day, an ad-hoc swim, or anything logged manually in Strava) now appear in the app instead of being silently dropped. They render as their own "UNSCHEDULED" card with real metrics (duration, distance, pace/power/HR, route map and lap graph when available), show up on the day they happened in both the Today and Calendar views, and their time counts toward your weekly sport totals. A rest day where you trained keeps the rest card but stacks the activity below it; race days stay clean (unscheduled work is hidden). Days with an unplanned activity get a distinct dashed marker so they read differently from a planned-and-completed day. As part of this, missed sessions on the Today tab now show the full scheduled workout (steps + intensity graph) with the same red-border/dimmed treatment as the Calendar tab, so you can always see what was planned even if you didn't do it (DRO-305).
- **`import-plan` Edge Function (V0 dogfood)** — Allows hand-crafted training plans to be ingested for `ebreard4@gmail.com` only. Validates every `template_id` against the workout library, validates all body fields (size cap, day names, phases, sports, types, profile ranges), snapshots the existing active plan before replace, and applies optional profile updates atomically. Foundation for importing the Olympic-distance plan in a follow-up step (DRO-299).
- **Workout intensity now displays the metric coaches actually prescribe** — Session cards across Home and Calendar now render targets in the form the workout was written in: HR caps as "HR 65-78% max (125-150 bpm)", power as exact watts ("240 W" or "240-260 W"), run pace as min/km ("4:11/km"), and swim pace as min/100m ("1:50/100m"). HR-target segments get their own intensity color gradient anchored on % HRmax (uses your profile's HRmax to derive the bpm band). Old session cards continue to work — these are additive display paths for sessions that use the richer intensity vocabulary (DRO-297).
- **Strength sessions render as notes-only cards** — Strength session cards on the Today and Calendar tabs now skip the workout graph and step list entirely (they were never meaningful for sets-and-reps work) and show the exercise list directly from the session notes. Sets×reps UI with proper graph rendering is deferred to a follow-up (DRO-297).
- **30 new workout templates with race-specific intensity targets** — The workout library now includes Olympic-distance-ready templates: bike VO2/threshold/sweet-spot/race-pace/over-under sessions specified in watts (e.g. 5×3min @ 290-300W); run Z2/tempo/threshold/VO2/race-pace sessions specified in pace per km (e.g. 5×400m @ 3:35/km) with distance-based active recovery; swim CSS/threshold/race-pace sessions specified in pace per 100m. Race day gets a proper Olympic template (1.5km / 40km / 10km). Existing templates are unchanged (DRO-298).
