...
| `import-plan` | `supabase/functions/import-plan/` | POST: Auth → email allowlist gate (V0: `ebreard4@gmail.com` only, 403 otherwise) → validates body schema (plan, weeks, sessions) → validates all `template_id`s against `workout-library.json` (400 on unknown IDs) → conflict check (409 if plan exists and `replace !== true`) → materialises `structure` JSONB for each session in TypeScript (using shared `materialize()`) → calls `import_plan_atomic` RPC (snapshot → delete → insert). Returns `{plan_id, snapshot_id, weeks_inserted, sessions_inserted}`. JWT validated via `auth.getUser()`. (DRO-299). |
